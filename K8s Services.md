...
* NodePort
	...
	* Differences between Port, NodePort and targetPort
		* **Port (`port`):** The port exposed by the service _inside_ the cluster. Other pods use this port to communicate with the service.
		...
		- **Node Port (`nodePort`):** A port opened on _every_ node in the cluster (usually 30000-32767). It allows external traffic from outside the cluster to access the service
* Cluster IP
